...
public interface WorkflowExecutionRepository extends JpaRepository<WorkflowExecution, UUID> {
    ...
    @Modifying
    @org.springframework.data.jpa.repository.Query("UPDATE WorkflowExecution execution SET execution.status = :runningStatus, execution.startedAt = CURRENT_TIMESTAMP, execution.errorMessage = null WHERE execution.id = :id AND execution.status = :pendingStatus AND execution.deleted = false")
    int claimPending(
        @Param("id") UUID id,
