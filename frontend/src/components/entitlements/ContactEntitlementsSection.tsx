...
interface ContactEntitlementsSectionProps {
  contactId: string;
}

export function ContactEntitlementsSection({ contactId }: ContactEntitlementsSectionProps) {
  const { data, isLoading } = useEntitlements({ contactId, size: 5 });
    ...
    <Card>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">No entitlements linked to this contact yet.</p>
        ) : (
