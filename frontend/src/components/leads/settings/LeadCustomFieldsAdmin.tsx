...
import {
  useCreateLeadCustomField,
  useDeleteLeadCustomField,
  useLeadCustomFields,
  useUpdateLeadCustomField,
...
const OPTION_TYPES: LeadFieldType[] = ["SELECT", "MULTISELECT"];
...
const emptyForm: LeadCustomFieldCreateRequest = {
  fieldKey: "",
  fieldLabel: "",
  fieldType: "TEXT",
  isRequired: false,
...
export function LeadCustomFieldsAdmin() {
  const { data: fields, isLoading } = useLeadCustomFields();
  const createMutation = useCreateLeadCustomField();
  const updateMutation = useUpdateLeadCustomField();
  const deleteMutation = useDeleteLeadCustomField();

  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<LeadCustomFieldResponse | null>(null);
  const [form, setForm] = useState<LeadCustomFieldCreateRequest>(emptyForm);
  const [toDelete, setToDelete] = useState<LeadCustomFieldResponse | null>(null);
  ...
  function openEdit(field: LeadCustomFieldResponse) {
    setEditing(field);
    setForm({
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <Field>
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setForm({
                    ...form,
                    fieldType: v as LeadFieldType,
                    options: OPTION_TYPES.includes(v as LeadFieldType)
                      ? form.options?.length
                        ? form.options
                        : [{ label: "", value: "" }]
                      : [],
          ...
          <DialogFooter>
            ...
            <Button
              onClick={handleSave}
              disabled={
                isPending || !form.fieldLabel.trim() || !form.fieldKey.trim()
              }
            >
      ...
      <AlertDialog open={!!toDelete} onOpenChange={() => setToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Remove &quot;{toDelete?.fieldLabel}&quot;? Existing lead data for this key will remain in JSONB.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
